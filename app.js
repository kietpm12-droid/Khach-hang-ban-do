...
async function init() {
  if (SUPABASE_URL.startsWith("DAN_")) {
    $("loginMsg").textContent =
      "Bạn chưa cấu hình Supabase trong app.js.";
    return;
  }
  const {
    data: { session }
...
$("gpsBtn").onclick = () => {
  ...
  navigator.geolocation.getCurrentPosition(
    p => {
      $("gpsStatus").dataset.lat =
        p.coords.latitude;
      $("gpsStatus").dataset.lng =
        p.coords.longitude;
      $("gpsStatus").textContent =
...
// =========================
// CHỤP ẢNH NHÀ KHÁCH HÀNG
// =========================
async function uploadHousePhoto(file, userId) {
  if (!file) {
    return null;
  }
  const extension =
...
$("saveBtn").onclick = async () => {
  ...
  // GPS
  const lat =
    parseFloat($("gpsStatus").dataset.lat);
  const lng =
    parseFloat($("gpsStatus").dataset.lng);
  // ẢNH
  const photoInput =
    $("housePhoto");
  const photoFile =
